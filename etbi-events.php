...
if ( ! class_exists( 'WPEMS' ) ) {
	...
	final class WPEMS {
		...
		public function define_constants() {
			...
			$this->set_define( 'WPEMS_VER', '2.1.7.3' );
		...
		public function init_hooks() {
			...
			add_filter( 'wpems_account_url', array( $this, 'bp_account_url' ), 10, 1 );
		}
		...
		public function bp_add_new_item() {
			if( function_exists('bp_is_active') ) {
				...
				$account_url = trailingslashit( $bp->loggedin_user->domain . bp_get_settings_slug() );
		...
		public function account_screens() {
			...
			add_action( 'bp_template_content', array( $this, "bp_tab_bookings_content" ) );
			bp_core_load_template( apply_filters( 'bp_core_template_plugin', 'members/single/plugins' ) );

		}

		public function bp_tab_bookings_content() {
			...
			echo wpems_get_template_content( 'shortcodes/user-account.php', $args );
		}

		public function bp_account_url( $url ) {
			if( ! function_exists('bp_is_active') ) {
				return $url;
			}

			if( bp_is_active('settings') ) {
				$url = bp_loggedin_user_domain() . bp_get_settings_slug();
				$url .= '/bookings/';
			}
		 ...
		 * @return WPEMS
		 */
		public static function bp_start() {

			$bp = buddypress();

			if ( empty( $bp->wpems ) ) {

				$bp->wpems = self::instance();

			}

			$bp->wpems->_include( 'inc/class-wpems-group-extension.php' );
	...
	add_action( 'bp_include', array( 'WPEMS', 'bp_start' ) );
