...
class WPEMS_Custom_Post_Types {
	...
	public function event_columns( $columns ) {
		...
		$columns['csv']			= __( 'CSV', 'wp-events-manager' );
	...
	public function event_column_content( $column, $post_id ) {
		...
		switch ( $column ) {
			...
			case 'csv':
				...
				$csv_link = wp_nonce_url( add_query_arg( 'etbi_participant_csv', $event_slug, get_the_permalink( $post_id ) ), '_etbi_download_event' . $post_id . '_csv', '_etbi_download_csv' );

				if ( $event->booked_quantity() > 0 ) {
					echo '<a href="'.$csv_link.'" class="event_auth_event_type button">' . __( 'Participants CSV', 'wp-events-manager' ) . '</a>';
				} else {
					echo __( 'No participants', 'wp-events-manager' );
				}
				break;
			case 'booked_slot':
