...
class WPEMS_Booking {
	...
	public function create_booking( $args = array(), $payment ) {
		...
		if( is_user_logged_in() ) {
			...
			$booking_id = wp_insert_post( array(
				...
				'post_excerpt' => sprintf( __( '%s booking event %s with %s slot', 'wp-events-manager' ), $user->user_nicename, $args['event_id'], $args['qty'] ),
			...
			$booking_id = wp_insert_post( array(
				'post_title'   => sprintf( __( '%s booking event %s', 'wp-events-manager' ), $user_nicename, $args['event_id'] ),
				'post_content' => sprintf( __( '%s booking event %s with %s slot', 'wp-events-manager' ), $user_nicename, $args['event_id'], $args['qty'] ),
				'post_excerpt' => sprintf( __( '%s booking event %s with %s slot', 'wp-events-manager' ), $user_nicename, $args['event_id'], $args['qty'] ),
				'post_status'  => 'ea-pending',
				'post_type'    => 'event_auth_book'
			) );
	...
	public function update_status( $status = 'ea-completed' ) {
		...
		if ( !$this->post || !$this->ID ) {
			throw new Exception( sprintf( __( 'Booking ID #%s does not exist.', 'wp-events-manager' ), $this->ID ) );
		}
