...
if( ! function_exists('wpems_is_admin_page') ) {

	function wpems_is_edit_page($new_edit = null){
	    global $pagenow;
...
if ( ! function_exists( 'wpems_add_property_countdown' ) ) {
	 ...
	 * @return string
	 */
	function wpems_get_start_time( $format = 'Y-m-d H:i', $post = null, $l10 = true ) {
		$current_time = current_time( 'timestamp' );
		$start        = wpems_event_start( 'Y-m-d H:i', $post );
		$end          = wpems_event_end( 'Y-m-d H:i', $post );
	 ...
	 * @return string
	 */
	function wpems_event_location_link( $post = null ) {
		if ( ! $post ) {
			$post = get_post();
		}

		return get_post_meta( $post->ID, 'tp_event_location_link', true );
	}
...
//add_action( 'tp_event_loop_event_extra', 'wpems_downloadable_participant_csv_button' );
if ( ! function_exists( 'wpems_downloadable_participant_csv_button' ) ) {

	function wpems_downloadable_participant_csv_button() {
		if( ! is_user_logged_in() && ! current_user_can('upload_files') ) {
			return;
		}
		wpems_get_template( 'loop/downloadable-csv-button.php' );
	}

}

add_filter('single_template', 'wpems_download_participant_csv', 1);
if( ! function_exists('wpems_download_participant_csv') ) {
	function wpems_download_participant_csv( $template ) {
		...
		$template = wpems_get_template( 'loop/downloadable-csv.php' );

		return $template;
	}
}

if( ! function_exists('wpems_apply_coupon_code') ) {
	function wpems_apply_coupon_code( $price, $coupon_code = null ) {

		if( is_null( $coupon_code ) ) {
			return $price;
		}

		$coupon_codes = wpems_get_coupon_codes();

		if( array_key_exists( $coupon_code, $coupon_codes ) ) {

			$discount = $coupon_codes[ $coupon_code ][ 'discount' ];
			$discount_price = $price * $discount;
			$final_price = $price - intval( $discount_price );
...
if( ! function_exists( 'wpems_get_coupon_codes' ) ) {

	function wpems_get_coupon_codes() {

		return apply_filters( 'wpems_get_coupon_codes', array(
		...
		if ( is_admin() && ! isset( $_GET['orderby'] ) ) {
			...
			if ( isset( $wp_query->query['post_type'] ) ) {
				...
				if ( in_array( $post_type, array( 'tp_event', 'event_auth_book' ) ) ) {
					if( ! wpems_is_edit_page() ) {
						//$wp_query->set( 'meta_key', 'tp_event_date_start' );
...
if( ! function_exists( 'wpems_register_group_extension' ) ) {

function wpems_register_group_extension() {

	bp_register_group_extension( 'WPEMS_Group_Extension' );

}

add_action( 'bp_init', 'wpems_register_group_extension' );

}

if( ! function_exists( 'wpems_group_update_group_events' ) ) {

	function wpems_group_update_group_events( $group_id, $event_ids = array() ) {
		...
		if( isset( $event_ids ) ) {
			...
			if( is_string( $event_ids ) ) {
				...
				$events = maybe_serialize( $event_ids );
			...
			$event_ids = wpems_get_group_event_ids( $event_ids );

			foreach ( $event_ids as $event_id ) {

				$prev_value = get_post_meta( $event_id, 'tp_event_group_parent', $group_id );
				
				wpems_add_group_association( $event_id, $group_id, $prev_value );

			}

			groups_update_groupmeta( $group_id, '_etbi_group_event_ids', $events );
...
if( ! function_exists( 'wpems_group_delete_group_events' ) ) {

	function wpems_group_delete_group_events( $group_id ) {

		$event_ids = maybe_unserialize( groups_get_groupmeta( $group_id, '_etbi_group_event_ids', true ) );
		...
		if( is_array( $event_ids ) && ! empty( $event_ids ) ) {
			...
			foreach ( $event_ids as $event_id ) {
				...
				wpems_delete_group_association( $event_id, $group_id );
...
if( ! function_exists('wpems_get_group_event_ids') ) {

	function wpems_get_group_event_ids( $group_event_ids = array() ) {

		if( ! is_array( $group_event_ids ) || empty( $group_event_ids ) ) {
			return false;
		}

		$event_ids = array();

		foreach ( $group_event_ids as $event_id ) {
			...
			if( $is_program ) {
				...
				$program_event_ids = (array) maybe_unserialize( get_post_meta( $event_id, 'tp_event_event_ids', true ) );

				foreach ( $program_event_ids as $program_event_id ) {

					array_push( $event_ids, $program_event_id );
...
if( ! function_exists('wpems_add_group_association') ) {

	function wpems_add_group_association( $event_id, $group_id, $prev_value = null ) {

		update_post_meta( $event_id, 'tp_event_group_parent', $group_id, $prev_value );
		
	}

}

if( ! function_exists('wpems_delete_group_association') ) {

	function wpems_delete_group_association( $event_id, $group_id ) {

		delete_post_meta( $event_id, 'tp_event_group_parent', $group_id );

	}

}

if( ! function_exists('wpems_event_is_program') ) {

	function wpems_event_is_program( $event_id ) {
...
if( ! function_exists('wpems_user_has_paid_for_program') ) {

	function wpems_user_has_paid_for_program( $user_id, $program_id ) {

		if( wpems_event_is_program( $program_id ) ) {

			$event   = WPEMS_Event::instance( $program_id );
...
if( ! function_exists('wpems_get_program_id') ) {

	function wpems_get_program_id_from_child_event( $event_id ) {

		$program_parent = get_post_meta( $event_id, '_tp_event_program_parent' );

		if( ! isset( $program_parent ) || ( empty( $program_parent ) || is_null( $program_parent ) ) ) {

			return null;

		} else if( wpems_event_is_program( $program_parent ) ) {
...
if( ! function_exists( 'wpems_event_belongs_to_program' ) ) {

	function wpems_event_belongs_to_program( $event_id ) {

		$is_program = wpems_event_is_program( $event_id );
		...
		if( isset( $program_parent ) && $program_parent > 0 && wpems_event_is_program( $program_parent ) ) {
