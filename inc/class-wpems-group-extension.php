 ...
 */
class WPEMS_Group_Extension extends BP_Group_Extension {
	...
	public function __construct() {
		$is_public_group = true; //$this->is_public_group();
		$admin_screen    = array();
		...
		parent::init(  array(
			...
			'screens'           => array(
				...
				'edit' => array(
					'enabled' => false, //$is_public_group && ( bp_current_user_can( 'bp_moderate' ) ),
				),
			),
		) );
	...
	private function setup_hooks() {
		...
		add_action( 'bp_before_group_body', array( $this, 'home_page_events' ), 20 );

		add_filter( 'etbi_progress_chart_l18n', array( $this, 'add_events_to_progress_chart' ), 20, 1 );
	...
	public function home_page_events() {
		...
		if ( bp_is_groups_component() && bp_is_group_home() ) {
			...
			$query_args = array( 'post__in' => $event_ids, 'post_type' => 'tp_event', 'orderby' => 'meta_value', 'meta_key' => 'tp_event_date_start', 'order' => 'ASC' );

			$template_name = 'buddypress/groups/horizontal-timeline.php';
			$args = array(

				'query' 				=> new WP_Query( $query_args ),
				'event_ids'				=> $event_ids,
				'group_events_array'	=> $group_events_array,
				'current_date' 			=> date( 'd/m/Y', time() ),
				'group_slug'			=> $group_slug

			);

			echo wpems_get_template_content( $template_name, $args );
	...
	public function add_events_to_progress_chart( $l18n ) {
		...
		if ( bp_is_groups_component() ) {
			...
			$query_args = array( 'post__in' => $event_ids, 'post_type' => 'tp_event', 'orderby' => 'meta_value', 'meta_key' => 'tp_event_date_start', 'order' => 'ASC', 'posts_per_page' => -1 );
			...
			if( $args['query']->have_posts() ) {
				...
				while( $args['query']->have_posts() ) {
					...
					$date_time = $date_start . ' ' . $time_start;
					...
					$data_date = date( 'Y-m-d H:i:s', strtotime( $date_time ) );
					$date_key = date( 'Y-m-d', strtotime( $date_start ) );

					if( $date_key >= $l18n['from'] && $date_key <= $l18n['until'] ) {

						$events[ $date_key ] = array( 'date' => $data_date, 'title' => get_the_title(), 'average' => 0 );
	...
	public function edit_screen_save( $group_id = null ) {
		...
		check_admin_referer( 'groups_edit_save_' . $this->slug, 'etbi' );

		$group_id = ! empty( $group_id ) ? $group_id : bp_get_current_group_id();

		// Update the visibility level.
		if ( isset( $_POST['_etbi_group_event_ids'] ) ) {

			if( empty( $_POST['_etbi_group_event_ids'] ) ) {

				wpems_group_delete_group_events( $group_id );

			} else {

				wpems_group_update_group_events( $group_id, $_POST['_etbi_group_event_ids'] );
		...
		bp_core_add_message( __( 'Settings saved successfully', 'etbi' ) );
	...
	public function admin_screen( $group_id = null ) {
		...
		<div id="apgc-admin">
			<h4><?php esc_html_e( 'Events', 'etbi' );?></h4>

			<!-- <ul><?php //apgc_group_visibility_options( $group_id ) ;?></ul> -->

			<?php echo require_once( WPEMS_INC . 'admin/views/metaboxes/group-events-settings.php' ); ?>

			<?php wp_nonce_field( 'groups_edit_save_' . $this->slug, 'etbi' ); ?>
		</div>
		<?php
	}
