...
if ( $query->have_posts() ) { ?>
    ...
    <table>
        <thead>
        <th><?php _e( 'Ticket ID', 'wp-events-manager' ); ?></th>
        ...
        <?php foreach ( $query->posts as $post ) { ?>
            ...
            <tr>
                ...
                <td><?php printf( '%s', $booking->price > 0 ? wpems_format_price( $booking->price ) : __( 'Free', 'wp-events-manager' ) ) ?></td>
                <td><?php printf( '%s', wpems_format_price( floatval( $event->get_price() ), $booking->currency ) ) ?></td>
