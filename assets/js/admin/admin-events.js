(function ($) {
	...
	TP_Event_Admin.admin_meta_boxes = {
		...
		program: function() {
			var _program = $('#_program'),
				_event_ids = $('#_event_ids'),
				_program_event_selector = $('#_program_event_selector');

				_program.change( function( event ) {

			        if(this.checked) {
			            console.log( 'checked' );
			            _program_event_selector.removeClass('hidden');
			        } else {
			        	console.log( 'unchecked' );
			        	_program_event_selector.addClass('hidden');
			        }
